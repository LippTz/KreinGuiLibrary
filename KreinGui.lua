...
local function CreateSettingsMenu(sg, parentButton, onThemeChange, onKeybindChange, onResetFlags)
    ...
    Corner(themeFrame, 8)
    Stroke(themeFrame, T.ElementStr, 1)

    local themeLabel = Lbl(themeFrame, "Theme Preset", UDim2.new(1, -70, 0, 20), T.TextSec)
    ...
    local themeOptions = {"Default","Neon","Ocean","Blood","Purple","Gold","Rose"}
    local themePickOpen = false
    local themePickPop = Instance.new("Frame", sg)
    themePickPop.Size = UDim2.new(0, 120, 0, 0)
    themePickPop.BackgroundColor3 = T.ElementBG
    themePickPop.BorderSizePixel = 0
    themePickPop.ClipsDescendants = true
    themePickPop.Visible = false
    themePickPop.ZIndex = 200
    Corner(themePickPop, 8)
    Stroke(themePickPop, T.Accent, 1)
    local themeScroller = Instance.new("ScrollingFrame", themePickPop)
    ...
    for _, name in ipairs(themeOptions) do
        local opt = Instance.new("TextButton", themeScroller)
        opt.Size = UDim2.new(1, 0, 0, 30)
        opt.BackgroundColor3 = T.ElementHov
        opt.BorderSizePixel = 0
        opt.Text = name
        opt.TextSize = 11
        opt.TextColor3 = T.TextPri
        Corner(opt, 4)
        ...
        opt.MouseEnter:Connect(function() opt.BackgroundColor3 = T.TabHov end)
        opt.MouseLeave:Connect(function() opt.BackgroundColor3 = T.ElementHov end)
    end

    local function closeThemePick()
        themePickOpen = false
        Tw(themePickPop, {Size = UDim2.new(0, 120, 0, 0)}, 0.18)
        task.delay(0.2, function() themePickPop.Visible = false end)
    end
    local function openThemePick()
        ...
        themePickPop.Position = UDim2.new(0, x, 0, y)
        themePickPop.Size = UDim2.new(0, w, 0, 0)
        themePickPop.Visible = true
        themePickOpen = true
        Tw(themePickPop, {Size = UDim2.new(0, w, 0, 140)}, 0.22)
    end
    OnClick(themeBtn, function() if themePickOpen then closeThemePick() else openThemePick() end end)
    ...
    Corner(colorFrame, 8)
    Stroke(colorFrame, T.ElementStr, 1)

    local colorLabel = Lbl(colorFrame, "Custom Accent", UDim2.new(1, -70, 0, 20), T.TextSec)
    colorLabel.Position = UDim2.new(0, 10, 0, 4)
    colorLabel.TextSize = 11

    local colorSwatch = Instance.new("Frame", colorFrame)
    colorSwatch.Size = UDim2.new(0, 32, 0, 24)
    colorSwatch.Position = UDim2.new(1, -42, 0.5, -12)
    ...
    keyFrame.Size = UDim2.new(1, 0, 0, 44)
    keyFrame.BackgroundColor3 = T.ElementBG
    keyFrame.BorderSizePixel = 0
    Corner(keyFrame, 8)
    Stroke(keyFrame, T.ElementStr, 1)

    local keyLabel = Lbl(keyFrame, "Toggle GUI Key", UDim2.new(1, -70, 0, 20), T.TextSec)
    ...
    local function updateKeyDisplay()
        local name = tostring(currentKey):gsub("Enum.KeyCode.", "")
        keyBtn.Text = name
    end
    ...
    resetBtn.BackgroundColor3 = T.ElementHov
    resetBtn.BorderSizePixel = 0
    resetBtn.Text = "Reset All Flags"
    resetBtn.TextSize = 12
    resetBtn.Font = T.FontBold
    resetBtn.TextColor3 = T.TextPri
    Corner(resetBtn, 8)
    ...
    local function updatePanelPosition()
        local btnPos = parentButton.AbsolutePosition
        local btnSize = parentButton.AbsoluteSize
        local x = btnPos.X + btnSize.X + 4
        local y = btnPos.Y
        local vp = workspace.CurrentCamera.ViewportSize
        local w = 240
        if x + w > vp.X - 4 then x = btnPos.X - w - 4 end
        panel.Position = UDim2.new(0, x, 0, y)
        panel.Size = UDim2.new(0, w, 0, math.min(300, vp.Y - y - 10))
    end
    ...
    local function toggle()
        ...
        if visible then
            ...
            Tw(panel, {Size = UDim2.new(0, 240, 0, panel.Size.Y.Offset)}, 0.25)
        else
    ...
    UserInput.InputBegan:Connect(function(i)
        ...
        task.defer(function()
            ...
            if not inPanel and not inBtn then toggle() end
        end)
    end)
...
function KreinGui:CreateWindow(cfg)
    ...
    ABar.Position = UDim2.new(0, 0, 0, 52)
    ...
    if sub ~= "" then
        ...
        SL.Position = UDim2.new(0, 52, 0, 28)
        SL.Font = T.FontFace
        SL.TextSize = 11
    end
    ...
    local avatarRound = Instance.new("Frame", H)
    avatarRound.Size = UDim2.new(0, 28, 0, 28)
    avatarRound.Position = UDim2.new(1, -170, 0.5, -14)
    avatarRound.BackgroundColor3 = T.WindowBG
    avatarRound.BorderSizePixel = 0
    avatarRound.ZIndex = 6
    Cor(avatarRound, 14)
    Str(avatarRound, T.Accent, 1)

    local avatarImage = Instance.new("ImageLabel", avatarRound)
    ...
    local nameLabel = Lbl(H, playerName, UDim2.new(0, 80, 0, 20), T.TextPri)
    nameLabel.Position = UDim2.new(1, -84, 0.5, -10)
    nameLabel.TextXAlignment = Enum.TextXAlignment.Right
    nameLabel.TextSize = 11
    nameLabel.Font = T.FontBold

    -- Tombol Menu ☰
    local menuBtn = Instance.new("TextButton", H)
    menuBtn.Size = UDim2.new(0, 30, 0, 30)
    menuBtn.Position = UDim2.new(1, -125, 0.5, -15)
    menuBtn.BackgroundColor3 = T.ElementHov
    menuBtn.BorderSizePixel = 0
    menuBtn.Text = "☰"
    menuBtn.TextSize = 18
    menuBtn.Font = T.FontBold
    ...
    Mb.Position = UDim2.new(1, -86, 0.5, -15)
    ...
    local TBIcon = Lbl(ToggleBtn, "◀", UDim2.new(1,0,0,16), T.Accent)
    TBIcon.Position = UDim2.new(0,0,0.5,-26)
    TBIcon.Font = T.FontBold
    TBIcon.TextSize = 10
    TBIcon.TextXAlignment = Enum.TextXAlignment.Center

    local TBLabel = Lbl(ToggleBtn, "K", UDim2.new(1,0,0,14), T.Accent)
    TBLabel.Position = UDim2.new(0,0,0.5,-7)
    TBLabel.Font = T.FontBold
    TBLabel.TextSize = 13
    TBLabel.TextXAlignment = Enum.TextXAlignment.Center

    local TBSub = Lbl(ToggleBtn, "GUI", UDim2.new(1,0,0,12), Color3.new(T.Accent.R*0.65, T.Accent.G*0.65, T.Accent.B*0.65))
    ...
    local function loadSettings()
        ...
        if data.accent and not Presets[currentThemeName] then
            KreinGui:SetTheme({Accent = Color3.new(data.accent.r, data.accent.g, data.accent.b)})
            W:ReloadTheme()
        end
    ...
    function W:CreateTab(name)
        ...
        BBar.Name = "B"
        BBar.Size = UDim2.new(0,3,0.55,0)
        BBar.Position = UDim2.new(0,0,0.225,0)
        BBar.BackgroundColor3 = T.AccentHov
        ...
        Con.ScrollBarThickness = 3
        Con.ScrollBarImageColor3 = T.Accent
        ...
        local function rfl(flag, api, element, typ, defaultVal, minVal, maxVal, options)
            if flag and flag ~= "" then
                ...
                api._type = typ
                api._defaultValue = defaultVal
                if minVal then api._min = minVal end
                if maxVal then api._max = maxVal end
                if options then api._options = options end
            end
        end
        ...
        local function Card(h)
            ...
            c.BackgroundColor3 = T.ElementBG
            c.BorderSizePixel = 0
            c.LayoutOrder = nxt()
            c.ClipsDescendants = false
            Cor(c,8)
            Str(c, T.ElementStr, 1)
            return c
        end
        ...
        function Tab:CreateButton(cfg2)
            ...
            hov.Text = ""
            hov.AutoButtonColor = false
            Lbl(c, cfg2.Title or "Button", UDim2.new(1,-82,1,0))
            local rb = Instance.new("TextButton", c)
            rb.Size = UDim2.new(0,68,0,30)
            rb.Position = UDim2.new(1,-72,0.5,-15)
            rb.BackgroundColor3 = T.Accent
        ...
        function Tab:CreateToggle(cfg2)
            ...
            Tr.Size = UDim2.new(0,44,0,24)
            Tr.Position = UDim2.new(1,-48,0.5,-12)
            Tr.BackgroundColor3 = st and T.ToggleOn or T.ToggleOff
        ...
        function Tab:CreateSlider(cfg2)
            ...
            TBG.Size = UDim2.new(1,0,0,10)
            TBG.Position = UDim2.new(0,0,1,-18)
            TBG.BackgroundColor3 = T.ToggleOff
            TBG.BorderSizePixel = 0
            Cor(TBG,5)
            local p0 = (val-mn)/(mx-mn)
            local TF = Instance.new("Frame", TBG)
            TF.Size = UDim2.new(p0,0,1,0)
            TF.BackgroundColor3 = T.Accent
            TF.BorderSizePixel = 0
            Cor(TF,5)
            local Kn = Instance.new("Frame", TBG)
            Kn.Size = UDim2.new(0,20,0,20)
            Kn.Position = UDim2.new(p0,-10,0.5,-10)
            Kn.BackgroundColor3 = Color3.fromRGB(255,255,255)
            Kn.BorderSizePixel = 0
            Kn.ZIndex = 3
            Cor(Kn,10)
        ...
        function Tab:CreateTextBox(cfg2)
            ...
            IF.Position = UDim2.new(0,0,1,-36)
            IF.BackgroundColor3 = T.WindowBG
        ...
        function Tab:CreateDropdown(cfg2)
            ...
            SF.Size = UDim2.new(0,90,0,28)
            SF.Position = UDim2.new(1,-90,0.5,-14)
            SF.BackgroundColor3 = T.WindowBG
            SF.BorderSizePixel = 0
            Cor(SF,6)
            Str(SF, T.ElementStr, 1)
            local SL = Lbl(SF, sel, UDim2.new(1,-18,1,0), T.TextPri)
            SL.Position = UDim2.new(0,6,0,0)
            SL.Font = T.FontFace
            SL.TextSize = 11
            local Arr = Lbl(SF, "▼", UDim2.new(0,14,1,0), T.TextMut, Enum.TextXAlignment.Center)
            Arr.Position = UDim2.new(1,-16,0,0)
            Arr.TextSize = 12
        ...
        function Tab:CreateMultiDropdown(cfg2)
            ...
            local SF = Instance.new("Frame", c)
            SF.Size = UDim2.new(0,100,0,28)
            SF.Position = UDim2.new(1,-100,0.5,-14)
            SF.BackgroundColor3 = T.WindowBG
            SF.BorderSizePixel = 0
            Cor(SF,6)
            Str(SF, T.ElementStr, 1)
        ...
        function Tab:CreateInputNumber(cfg2)
            ...
            Mins.Size = UDim2.new(0,30,1,0)
            Mins.BackgroundColor3 = T.ElementHov
            Mins.BorderSizePixel = 0
            Mins.Text = "−"
            Mins.TextSize = 16
            Mins.Font = T.FontBold
            Mins.TextColor3 = T.TextPri
            Mins.AutoButtonColor = false
            Cor(Mins,6)
            local VF = Instance.new("Frame", Row)
            VF.Size = UDim2.new(0,50,1,0)
            VF.Position = UDim2.new(0,34,0,0)
            VF.BackgroundColor3 = T.WindowBG
            VF.BorderSizePixel = 0
            Cor(VF,6)
            Str(VF, T.ElementStr, 1)
            ...
            Plus.BackgroundColor3 = T.ElementHov
            Plus.BorderSizePixel = 0
            Plus.Text = "+"
            Plus.TextSize = 16
            Plus.Font = T.FontBold
            Plus.TextColor3 = T.TextPri
            ...
            function api:Get() return val end
            OnClick(Mins, function()
                val = math.clamp(val - step, mn, mx); updN()
                Tw(Mins, {BackgroundColor3 = T.AccentDark}, 0.1)
                task.delay(0.15, function() Tw(Mins, {BackgroundColor3 = T.ElementHov}, 0.15) end)
            end)
        ...
        function Tab:CreateProgressBar(cfg2)
            ...
            TR.Size = UDim2.new(1,0,0,20)
            TR.BackgroundTransparency = 1
            local tl = Lbl(TR, cfg2.Title or "Progress", UDim2.new(1,-42,1,0))
            tl.Font = T.FontFace
            tl.TextSize = 13
            local pl = Lbl(TR, val.."%", UDim2.new(0,40,1,0), T.Accent, Enum.TextXAlignment.Right)
            pl.Position = UDim2.new(1,-40,0,0)
            pl.Font = T.FontBold
            local Trk = Instance.new("Frame", c)
            Trk.Size = UDim2.new(1,0,0,12)
            Trk.Position = UDim2.new(0,0,1,-18)
            Trk.BackgroundColor3 = T.ToggleOff
            Trk.BorderSizePixel = 0
            Cor(Trk,6)
            local Fil = Instance.new("Frame", Trk)
            Fil.Size = UDim2.new(val/100,0,1,0)
            Fil.BackgroundColor3 = T.Accent
            Fil.BorderSizePixel = 0
            Cor(Fil,6)
            local Sh = Instance.new("Frame", Fil)
        ...
        function Tab:CreateColorPicker(cfg2)
            ...
            local cH,cS,cV = toHSV(col)
            local tH,tS,tV = cH,cS,cV
            local open = false
            local c = Card(44)
            Pad(c,0,0,12,12)
            ...
            Sw.Position = UDim2.new(1,-38,0.5,-12)
            Sw.BackgroundColor3 = col
            Sw.BorderSizePixel = 0
            Cor(Sw,5)
            Str(Sw,T.ElementStr,1)
            local PW,PH = 210,270
            local Pop = Instance.new("Frame", SG)
            Pop.Size = UDim2.new(0,PW,0,0)
            Pop.BackgroundColor3 = T.ElementBG
            Pop.BorderSizePixel = 0
            Pop.ClipsDescendants = true
            Pop.Visible = false
            Pop.ZIndex = 160
            Cor(Pop,8)
            Str(Pop,T.ElementStr,1)
            -- SV
            local SV = Instance.new("Frame", Pop)
            SV.Size = UDim2.new(1,-16,0,110)
            SV.Position = UDim2.new(0,8,0,8)
            SV.BackgroundColor3 = HSV(tH,1,1)
            SV.BorderSizePixel = 0
            SV.ZIndex = 161
            Cor(SV,6)
            local WL = Instance.new("Frame", SV)
            WL.Size = UDim2.new(1,0,1,0)
            WL.BackgroundColor3 = Color3.fromRGB(255,255,255)
            WL.BorderSizePixel = 0
            WL.ZIndex = 162
            Cor(WL,6)
            local wg = Instance.new("UIGradient", WL)
            wg.Transparency = NumberSequence.new({NumberSequenceKeypoint.new(0,0),NumberSequenceKeypoint.new(1,1)})
            local BL = Instance.new("Frame", SV)
            BL.Size = UDim2.new(1,0,1,0)
            BL.BackgroundColor3 = Color3.fromRGB(0,0,0)
            BL.BorderSizePixel = 0
            BL.ZIndex = 163
            Cor(BL,6)
            local bg = Instance.new("UIGradient", BL)
            bg.Transparency = NumberSequence.new({NumberSequenceKeypoint.new(0,1),NumberSequenceKeypoint.new(1,0)})
            bg.Rotation = 90
            local SVC = Instance.new("Frame", SV)
            SVC.Size = UDim2.new(0,12,0,12)
            SVC.Position = UDim2.new(tS,-6,1-tV,-6)
            SVC.BackgroundColor3 = Color3.fromRGB(255,255,255)
            SVC.BorderSizePixel = 0
            SVC.ZIndex = 165
            Cor(SVC,6)
            Str(SVC,Color3.fromRGB(0,0,0),1)
            -- Hue
            local HB = Instance.new("Frame", Pop)
            HB.Size = UDim2.new(1,-16,0,16)
            HB.Position = UDim2.new(0,8,0,124)
            HB.BackgroundColor3 = Color3.fromRGB(255,255,255)
            HB.BorderSizePixel = 0
            HB.ZIndex = 161
            Cor(HB,4)
            local HI = Instance.new("ImageLabel", HB)
            HI.Size = UDim2.new(1,0,1,0)
            HI.BackgroundTransparency = 1
            HI.Image = "rbxassetid://698052001"
            HI.ZIndex = 162
            local HC = Instance.new("Frame", HB)
            HC.Size = UDim2.new(0,6,1,4)
            HC.Position = UDim2.new(tH,-3,0,-2)
            HC.BackgroundColor3 = Color3.fromRGB(255,255,255)
            HC.BorderSizePixel = 0
            HC.ZIndex = 163
            Cor(HC,3)
            Str(HC,Color3.fromRGB(0,0,0),1)
            -- Hex
            local HxF = Instance.new("Frame", Pop)
            HxF.Size = UDim2.new(1,-16,0,26)
            HxF.Position = UDim2.new(0,8,0,148)
            HxF.BackgroundColor3 = T.WindowBG
            HxF.BorderSizePixel = 0
            HxF.ZIndex = 161
            Cor(HxF,5)
            Str(HxF,T.ElementStr,1)
            Lbl(HxF,"#",UDim2.new(0,16,1,0),T.TextMut,Enum.TextXAlignment.Center).ZIndex = 162
            local HL = Lbl(HxF,toHex(col),UDim2.new(1,-18,1,0),T.TextPri)
            HL.Position = UDim2.new(0,18,0,0)
            HL.TextSize = 11
            HL.Font = T.FontBold
            HL.ZIndex = 162
            -- Preview
            local BP = Instance.new("Frame", Pop)
            BP.Size = UDim2.new(1,-16,0,14)
            BP.Position = UDim2.new(0,8,0,180)
            BP.BackgroundColor3 = col
            BP.BorderSizePixel = 0
            BP.ZIndex = 161
            Cor(BP,4)
            -- Buttons
            local BR = Instance.new("Frame", Pop)
            BR.Size = UDim2.new(1,-16,0,28)
            BR.Position = UDim2.new(0,8,0,200)
            BR.BackgroundTransparency = 1
            local OK = Instance.new("TextButton", BR)
            OK.Size = UDim2.new(0.5,-3,1,0)
            OK.BackgroundColor3 = T.Accent
            ...
            Cor(OK,6)
            local CL = Instance.new("TextButton", BR)
            CL.Size = UDim2.new(0.5,-3,1,0)
            CL.Position = UDim2.new(0.5,3,0,0)
            CL.BackgroundColor3 = T.ElementHov
            CL.BorderSizePixel = 0
            CL.Text = "Batal"
            CL.TextSize = 12
            CL.Font = T.FontBold
            CL.TextColor3 = T.TextSec
            ...
            local function updPrev()
                local tc = HSV(tH,tS,tV)
                SV.BackgroundColor3 = HSV(tH,1,1)
                SVC.Position = UDim2.new(tS,-6,1-tV,-6)
                HC.Position = UDim2.new(tH,-3,0,-2)
                HL.Text = toHex(tc)
                BP.BackgroundColor3 = tc
            end
            local function updPos()
                local vp = workspace.CurrentCamera.ViewportSize
                local ap = Sw.AbsolutePosition
                local x = math.max(4, math.min(ap.X-PW+34, vp.X-PW-4))
                local y = ap.Y+28
                if y+PH > vp.Y-4 then y = ap.Y-PH-4 end
                Pop.Position = UDim2.new(0,x,0,y)
            end
            local function closePop()
                open = false
                Tw(Pop, {Size = UDim2.new(0,PW,0,0)}, 0.18)
            ...
            OnClick(OK, function()
                ...
                cH,cS,cV = tH,tS,tV
                Sw.BackgroundColor3 = col
                pcall(cfg2.Callback or function() end, col)
                closePop()
            end)
            OnClick(CL, function()
                tH,tS,tV = cH,cS,cV
            ...
            local svd = false
            local SVB = Instance.new("TextButton", SV)
            SVB.Size = UDim2.new(1,0,1,0)
            SVB.BackgroundTransparency = 1
            SVB.Text = ""
            SVB.ZIndex = 166
            SVB.InputBegan:Connect(function(i)
                if isDown(i) then
                    svd = true
                    tS = math.clamp((i.Position.X - SV.AbsolutePosition.X) / SV.AbsoluteSize.X, 0, 1)
                    tV = 1 - math.clamp((i.Position.Y - SV.AbsolutePosition.Y) / SV.AbsoluteSize.Y, 0, 1)
                    updPrev()
                end
            end)
            UserInput.InputChanged:Connect(function(i)
                if svd and isMove(i) then
                    tS = math.clamp((i.Position.X - SV.AbsolutePosition.X) / SV.AbsoluteSize.X, 0, 1)
                    tV = 1 - math.clamp((i.Position.Y - SV.AbsolutePosition.Y) / SV.AbsoluteSize.Y, 0, 1)
                    updPrev()
                end
            ...
            local hud = false
            local HBtn = Instance.new("TextButton", HB)
            ...
            HBtn.InputBegan:Connect(function(i)
                if isDown(i) then
                    hud = true
                    tH = math.clamp((i.Position.X - HB.AbsolutePosition.X) / HB.AbsoluteSize.X, 0, 1)
                    updPrev()
                end
            end)
            UserInput.InputChanged:Connect(function(i)
                if hud and isMove(i) then
                    tH = math.clamp((i.Position.X - HB.AbsolutePosition.X) / HB.AbsoluteSize.X, 0, 1)
                    updPrev()
                end
            end)
            UserInput.InputEnded:Connect(function(i) if isDown(i) then hud = false end end)
            local TB3 = Instance.new("TextButton", c)
            TB3.Size = UDim2.new(1,0,1,0)
            TB3.BackgroundTransparency = 1
            TB3.Text = ""
            TB3.AutoButtonColor = false
            OnClick(TB3, function()
                if open then
                    ...
                    Pop.Visible = true
                    open = true
                    Tw(Pop, {Size = UDim2.new(0,PW,0,PH)}, 0.22)
                end
            end)
        ...
        function Tab:CreateKeybind(cfg2)
            ...
            Bdg.Size = UDim2.new(0,82,0,30)
            Bdg.Position = UDim2.new(1,-86,0.5,-15)
            Bdg.BackgroundColor3 = T.WindowBG
            Bdg.BorderSizePixel = 0
            Bdg.TextSize = 11
            Bdg.Font = T.FontBold
            Bdg.TextColor3 = T.Accent
            Bdg.AutoButtonColor = false
            Bdg.ZIndex = 5
            Cor(Bdg,5)
            Str(Bdg,T.ElementStr,1)
            local function kN(k)
            ...
            local function stopL()
                ...
                Bdg.TextColor3 = T.Accent
                Bdg.Text = "["..kN(key).."]"
                Tw(Bdg, {BackgroundColor3 = T.WindowBG}, 0.15)
            end
            OnClick(Bdg, function()
                if lstn then stopL(); return end
                lstn = true
                Bdg.Text = "[ ... ]"
                Bdg.TextColor3 = T.AccentHov
                Tw(Bdg, {BackgroundColor3 = T.TabHov}, 0.15)
            end)
            UserInput.InputBegan:Connect(function(i,gp)
                ...
                if gp then return end
                if i.UserInputType == Enum.UserInputType.Keyboard then
                    if i.KeyCode == Enum.KeyCode.Escape then stopL(); return end
                    key = i.KeyCode
                    stopL()
                    pcall(cfg2.Callback or function() end, key)
                end
