...
║                  Version 2.1.0 (Mobile Fix)                  ║
╚══════════════════════════════════════════════════════════════╝

  CHANGELOG v2.1.0:
  ✓ Fix drag mobile (Touch support)
  ✓ Fix slider mobile (Touch support)
  ✓ Fix dropdown tertimpa (ZIndex + parent ke SG)
  ✓ Fix ColorPicker touch support
  ✓ Fix semua interaksi untuk mobile & PC
...
─────────────────────────────────────────────────────────────
  ...
  QUICK START:
    ...
    -- Opsional: custom theme
    KreinGui:SetTheme({ Accent = Color3.fromRGB(255,100,50) })
    -- atau preset: KreinGui:UsePreset("Rose") / "Emerald" / "Amber" / "Midnight"
    ...
    Tab:CreateToggle({
        Title = "God Mode", Flag = "GodMode", Default = false,
        Callback = function(v) end,
    })

    Tab:CreateSlider({
        Title = "Walk Speed", Flag = "WalkSpeed",
    ...
    Tab:CreateDropdown({
        Title = "Tim", Flag = "Team",
        Options = {"Merah","Biru","Hijau"}, Default = "Merah",
        Callback = function(v) end,
    })

    Tab:CreateColorPicker({
        Title = "ESP Color", Flag = "ESPColor",
        Default = Color3.fromRGB(255,0,0),
        Callback = function(c) end,
    })

    Tab:CreateKeybind({
        Title = "Toggle GUI", Flag = "ToggleKey",
...
-- ============================================================
-- MOBILE DETECTION & INPUT HELPERS
-- ============================================================
-- Cek apakah input adalah sentuhan (Touch) atau mouse
local function isTouchInput(inp)
    return inp.UserInputType == Enum.UserInputType.Touch
end
local function isMouseInput(inp)
    return inp.UserInputType == Enum.UserInputType.MouseButton1
end
local function isPointerDown(inp)
    return isMouseInput(inp) or isTouchInput(inp)
end
...
local Presets = {
    Default = {
        Accent=Color3.fromRGB(99,102,241), ToggleOn=Color3.fromRGB(99,102,241),
        TabActive=Color3.fromRGB(99,102,241), WindowBG=Color3.fromRGB(28,28,32), HeaderBG=Color3.fromRGB(22,22,26),
    },
    Rose = {
        Accent=Color3.fromRGB(244,63,94), ToggleOn=Color3.fromRGB(244,63,94),
        TabActive=Color3.fromRGB(244,63,94), WindowBG=Color3.fromRGB(30,20,24), HeaderBG=Color3.fromRGB(22,14,18),
    },
    Emerald = {
        Accent=Color3.fromRGB(16,185,129), ToggleOn=Color3.fromRGB(16,185,129),
        TabActive=Color3.fromRGB(16,185,129), WindowBG=Color3.fromRGB(18,28,24), HeaderBG=Color3.fromRGB(12,20,18),
    },
    Amber = {
        Accent=Color3.fromRGB(245,158,11), ToggleOn=Color3.fromRGB(245,158,11),
        TabActive=Color3.fromRGB(245,158,11), WindowBG=Color3.fromRGB(28,24,16), HeaderBG=Color3.fromRGB(22,18,10),
    },
    Midnight = {
        Accent=Color3.fromRGB(139,92,246), ToggleOn=Color3.fromRGB(139,92,246),
        TabActive=Color3.fromRGB(139,92,246), WindowBG=Color3.fromRGB(15,15,25), HeaderBG=Color3.fromRGB(10,10,18),
    },
}
...
local function EnableDrag(frame, handle)
    ...
    local function onInputBegan(inp)
        -- [FIX MOBILE] deteksi Touch DAN Mouse
        if isPointerDown(inp) then
            dragging  = true
    ...
    local function onInputChanged(inp)
        if dragging then
            -- [FIX MOBILE] Touch & Mouse movement
            if isPointerMove(inp) then
                local pos = inp.UserInputType == Enum.UserInputType.Touch
                    and inp.Position or inp.Position
    ...
    UserInput.InputEnded:Connect(function(inp)
        if isPointerDown(inp) then dragging = false end
    end)
end
...
local function RGBtoHSV(c)    return Color3.toHSV(c) end
...
function KreinGui:SetTheme(ov)
    ...
    if ov.Accent then
        ...
        if Theme.ToggleOn == Theme.Accent then Theme.ToggleOn = ov.Accent end
        if Theme.TabActive == Theme.Accent then Theme.TabActive = ov.Accent end
    end
end
...
function KreinGui:CreateWindow(cfg)
    ...
    -- [FIX] IgnoreGuiInset agar posisi benar di mobile
    ...
    TL.Position=TitlePos; TL.Font=Enum.Font.GothamBold; TL.TextSize=15; TL.ZIndex=5
    ...
    MinBtn.Size=UDim2.new(0,32,0,32); MinBtn.Position=UDim2.new(1,-80,0.5,-16)
    ...
    -- [FIX MOBILE] Drag dengan Touch support
    ...
    Sep.Size=UDim2.new(0,1,1,0); Sep.Position=UDim2.new(0,TAB_W,0,0)
    Sep.BackgroundColor3=Theme.Separator; Sep.BorderSizePixel=0
    ...
    -- [FIX DROPDOWN] Content panel TIDAK ClipsDescendants, biar dropdown popup tidak terpotong
    ...
    ContentPanel.ClipsDescendants = false  -- [FIX] was true, caused dropdown/colorpicker clipping
    ...
    -- ── SAVE / LOAD ──────────────────────────────────────────
    local function serCol(c) return {r=c.R,g=c.G,b=c.B} end
    local function desCol(t) return Color3.new(t.r,t.g,t.b) end
    ...
    function WObj:SaveConfig()
        ...
        for flag, api in pairs(flagMap) do
            ...
            if typeof(v)=="Color3" then
                data[flag]={__type="Color3",value=serCol(v)}
        ...
        self:Notify(ok and "Config tersimpan!" or "Gagal simpan: "..tostring(err), 2)
    end
        ...
        for flag,val in pairs(data) do
            if flagMap[flag] then
                if type(val)=="table" and val.__type=="Color3" then
                    flagMap[flag]:Set(desCol(val.value))
                elseif type(val)=="table" and val.__type=="EnumItem" then
                    local parts=string.split(val.value,".")
                    local ok3,en=pcall(function() return Enum[parts[2]][parts[3]] end)
    ...
    function WObj:CreateTab(name)
        ...
        Btn.Name=("Tab_"..idx); Btn.Size=UDim2.new(1,-4,0,38)
        ...
        Bar.BackgroundColor3=Theme.AccentHover; Bar.BorderSizePixel=0; Bar.Visible=false
        Corner(Bar,2)

        local BtnLbl=Lbl(Btn,name,UDim2.new(1,-14,1,0),Theme.TabDefaultText)
        BtnLbl.Name="Lbl"; BtnLbl.Position=UDim2.new(0,10,0,0)
        BtnLbl.Font=Enum.Font.GothamMedium; BtnLbl.TextSize=12

        local function doTab() setActive(idx) end
        Btn.MouseButton1Click:Connect(doTab)
        Btn.TouchTap:Connect(doTab)
        ...
        -- [FIX] ScrollingFrame per-tab tidak ClipsDescendants agar popup tidak terpotong
        ...
        Content.ScrollBarImageColor3=Theme.Accent; Content.CanvasSize=UDim2.new(0,0,0,0)
        Content.AutomaticCanvasSize=Enum.AutomaticSize.Y
        Content.ClipsDescendants=false   -- [FIX] agar dropdown & colorpicker popup tidak terpotong
        ...
        local function Card(h)
            ...
            c.ClipsDescendants=false  -- [FIX] biar popup anak card tidak terpotong
        ...
        local function RegFlag(flag,api)
            if flag and flag~="" then
                flagMap[flag]=api; KreinGui.Flags[flag]=api
            end
        end
        ...
        function TObj:CreateButton(cfg2)
            ...
            rb.MouseButton1Click:Connect(doRun)
            rb.TouchTap:Connect(doRun)
        ...
        function TObj:CreateToggle(cfg2)
            ...
            local function doToggle() state=not state; upd() end
        ...
        -- ══════════════════════════════════════════════
        -- SLIDER  — [FIX MOBILE] Touch support
        -- ══════════════════════════════════════════════
        function TObj:CreateSlider(cfg2)
            ...
            -- [FIX MOBILE] Hit area lebih besar untuk touch
            local SBtn=Instance.new("TextButton",TBG)
            SBtn.Size=UDim2.new(1,0,0,36); SBtn.Position=UDim2.new(0,0,0.5,-18)
            ...
            local function updSlider(x)
                ...
                TFill.Size=UDim2.new(p,0,1,0); Knob.Position=UDim2.new(p,-9,0.5,-9)
            ...
            -- [FIX MOBILE] InputBegan: handle Touch & Mouse
            SBtn.InputBegan:Connect(function(inp)
                if isPointerDown(inp) then
                    sliding=true
                    updSlider(inp.Position.X)
                    -- Track touch movement langsung dari inp
                    inp.Changed:Connect(function()
                        if inp.UserInputState==Enum.UserInputState.End then
                            sliding=false
                        end
            ...
            -- [FIX MOBILE] InputChanged global: Touch & Mouse movement
            ...
            UserInput.InputEnded:Connect(function(inp)
                if isPointerDown(inp) then sliding=false end
            end)
        ...
        -- ══════════════════════════════════════════════
        -- DROPDOWN — [FIX] ZIndex & popup di SG langsung
        -- ══════════════════════════════════════════════
        function TObj:CreateDropdown(cfg2)
            ...
            SF.BackgroundColor3=Theme.WindowBG; SF.BorderSizePixel=0
            ...
            -- [FIX] Popup di-parent ke SG (ScreenGui) bukan ke Card
            -- Sehingga tidak akan di-clip oleh parent manapun
            local DF=Instance.new("Frame",SG)
            DF.Size=UDim2.new(0,100,0,0)
            DF.BackgroundColor3=Theme.ElementBG; DF.BorderSizePixel=0
            DF.ClipsDescendants=true; DF.Visible=false
            DF.ZIndex=150  -- [FIX] ZIndex tinggi agar tidak tertimpa apapun
            ...
            for i,opt in ipairs(opts) do
                ...
                ob.AutoButtonColor=false; ob.ZIndex=151; ob.LayoutOrder=i
                ...
                local function selectOpt()
                    sel=opt; SL.Text=opt; open=false
                    Tween(DF,{Size=UDim2.new(0,100,0,0)},0.15)
                ...
                ob.TouchTap:Connect(selectOpt)
            end

            local totalH=#opts*optH+8

            -- [FIX] Fungsi update posisi popup berdasarkan posisi SF di layar
            local function updatePopupPos()
                ...
                DF.Size=UDim2.new(0, math.max(absSize.X+10, 100), 0, 0)
            end
            ...
            local function doToggle()
                ...
                if open then
                    updatePopupPos()
                    DF.Visible=true
                    local w=math.max(SF.AbsoluteSize.X+10,100)
            ...
            TB2.MouseButton1Click:Connect(doToggle)
            TB2.TouchTap:Connect(doToggle)
            ...
            UserInput.InputBegan:Connect(function(inp)
                if open and isPointerDown(inp) then
                    task.wait()
                    if open then
                        open=false
                        Tween(DF,{Size=UDim2.new(0,DF.Size.X.Offset,0,0)},0.15)
        ...
        -- ══════════════════════════════════════════════
        -- COLOR PICKER — [FIX] Touch + popup di SG
        -- ══════════════════════════════════════════════
        function TObj:CreateColorPicker(cfg2)
            ...
            -- [FIX] Popup di SG langsung agar tidak terpotong
            ...
            Pop.ClipsDescendants=true; Pop.Visible=false; Pop.ZIndex=150
            ...
            SVBox.BackgroundColor3=HSVtoRGB(h,1,1); SVBox.BorderSizePixel=0; SVBox.ZIndex=151
            Corner(SVBox,6)
            ...
            SVImg.Image="rbxassetid://4155801252"; SVImg.ZIndex=152
            ...
            SVDark.Rotation=90; SVDark.ZIndex=153

            local SVCursor=Instance.new("Frame",SVBox)
            SVCursor.Size=UDim2.new(0,12,0,12); SVCursor.Position=UDim2.new(s,-6,1-v,-6)
            SVCursor.BackgroundColor3=Color3.fromRGB(255,255,255); SVCursor.BorderSizePixel=0; SVCursor.ZIndex=154
            ...
            HueBar.BackgroundColor3=Color3.fromRGB(255,255,255); HueBar.BorderSizePixel=0; HueBar.ZIndex=151
            Corner(HueBar,4)
            ...
            HueImg.Image="rbxassetid://698052001"; HueImg.ZIndex=152

            local HueCursor=Instance.new("Frame",HueBar)
            HueCursor.Size=UDim2.new(0,6,1,4); HueCursor.Position=UDim2.new(h,-3,0,-2)
            HueCursor.BackgroundColor3=Color3.fromRGB(255,255,255); HueCursor.BorderSizePixel=0; HueCursor.ZIndex=153
            Corner(HueCursor,3); Stroke(HueCursor,Color3.fromRGB(0,0,0),1)

            -- Hex
            local HexFrame=Instance.new("Frame",Pop)
            HexFrame.Size=UDim2.new(1,-16,0,26); HexFrame.Position=UDim2.new(0,8,0,144)
            HexFrame.BackgroundColor3=Theme.WindowBG; HexFrame.BorderSizePixel=0; HexFrame.ZIndex=151
            ...
            HexPre.TextSize=11; HexPre.Font=Enum.Font.GothamBold; HexPre.ZIndex=152

            local HexLbl=Lbl(HexFrame,ColorToHex(color),UDim2.new(1,-18,1,0),Theme.TextPrimary)
            HexLbl.Position=UDim2.new(0,18,0,0); HexLbl.TextSize=11; HexLbl.Font=Enum.Font.GothamBold; HexLbl.ZIndex=152

            local BotPrev=Instance.new("Frame",Pop)
            BotPrev.Size=UDim2.new(1,-16,0,14); BotPrev.Position=UDim2.new(0,8,0,174)
            BotPrev.BackgroundColor3=color; BotPrev.BorderSizePixel=0; BotPrev.ZIndex=151
            Corner(BotPrev,4)

            local api={}
            local function updateColor()
                color=HSVtoRGB(h,s,v)
                Swatch.BackgroundColor3=color; SVBox.BackgroundColor3=HSVtoRGB(h,1,1)
                SVCursor.Position=UDim2.new(s,-6,1-v,-6)
                HueCursor.Position=UDim2.new(h,-3,0,-2)
                HexLbl.Text=ColorToHex(color); BotPrev.BackgroundColor3=color
                pcall(cfg2.Callback or function()end,color)
            end

            -- [FIX] Popup posisi update
            local function updatePopPos()
                local absPos=Swatch.AbsolutePosition
                Pop.Position=UDim2.new(0,absPos.X-POP_W+34,0,absPos.Y+28)
            end

            -- SV drag — [FIX MOBILE] Touch & Mouse
            local svDrag=false
            local SVBtn=Instance.new("TextButton",SVBox)
            SVBtn.Size=UDim2.new(1,0,1,0); SVBtn.BackgroundTransparency=1
            SVBtn.BorderSizePixel=0; SVBtn.Text=""; SVBtn.ZIndex=155
            ...
            UserInput.InputChanged:Connect(function(inp)
                if svDrag and isPointerMove(inp) then
                    local rx=math.clamp((inp.Position.X-SVBox.AbsolutePosition.X)/SVBox.AbsoluteSize.X,0,1)
                    local ry=math.clamp((inp.Position.Y-SVBox.AbsolutePosition.Y)/SVBox.AbsoluteSize.Y,0,1)
                    s=rx; v=1-ry; updateColor()
                end
            end)
            UserInput.InputEnded:Connect(function(inp) if isPointerDown(inp) then svDrag=false end end)

            -- Hue drag — [FIX MOBILE]
            local hueDrag=false
            local HueBtn=Instance.new("TextButton",HueBar)
            HueBtn.Size=UDim2.new(1,0,1,0); HueBtn.BackgroundTransparency=1
            HueBtn.BorderSizePixel=0; HueBtn.Text=""; HueBtn.ZIndex=154
            ...
            UserInput.InputChanged:Connect(function(inp)
                if hueDrag and isPointerMove(inp) then
                    h=math.clamp((inp.Position.X-HueBar.AbsolutePosition.X)/HueBar.AbsoluteSize.X,0,1)
                    updateColor()
                end
            end)
            UserInput.InputEnded:Connect(function(inp) if isPointerDown(inp) then hueDrag=false end end)
            ...
            TogBtn.TouchTap:Connect(doToggle)
            ...
            function api:Set(nc) color=nc; h,s,v=RGBtoHSV(nc); updateColor() end
        ...
        function TObj:CreateKeybind(cfg2)
            ...
            Badge.Size=UDim2.new(0,80,0,28); Badge.Position=UDim2.new(1,-84,0.5,-14)
