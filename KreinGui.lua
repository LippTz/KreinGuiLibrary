...
╚══════════════════════════════════════════════════════════════╝
  ...
  CHANGELOG v2.3.0:
  ✓ Fix ABar (garis accent) ngambang saat minimize
  ✓ Fix scroll konten tidak konflik dengan tap tombol
  ✓ Fix ColorPicker gelap / tertutup overlay hitam
  ✓ ColorPicker SV box sekarang menggunakan gradient yang benar
  ✓ Tambah contoh 2 tab di quick start
  ...
  QUICK START:
    ...
    Tab1:CreateLabel("Selamat datang di Tab 1!")
    Tab1:CreateButton({ Title="Kill All", Callback=function() print("Kill All!") end })
    Tab1:CreateToggle({ Title="God Mode", Flag="GodMode", Default=false, Callback=function(v) print(v) end })
    Tab1:CreateSlider({ Title="WalkSpeed", Flag="WS", Min=16, Max=100, Default=16,
        Callback=function(v) game.Players.LocalPlayer.Character.Humanoid.WalkSpeed=v end })
    Tab1:CreateTextBox({ Title="Target", Flag="Target", Placeholder="Nama target...",
        Callback=function(v) print(v) end })
    Tab1:AddSeparator()
    Tab1:CreateDropdown({ Title="Tim", Flag="Team",
        Options={"Merah","Biru","Hijau","Kuning"}, Default="Merah",
        Callback=function(v) print(v) end })

    -- Tab 2
    local Tab2 = Win:CreateTab("Visual")
    Tab2:CreateLabel("Pengaturan visual ada di sini.")
    Tab2:CreateColorPicker({ Title="ESP Color", Flag="ESPColor",
        Default=Color3.fromRGB(255,0,0), Callback=function(c) print(c) end })
    Tab2:CreateToggle({ Title="Show ESP", Flag="ShowESP", Default=true, Callback=function(v) end })
    Tab2:CreateKeybind({ Title="Toggle GUI", Flag="GUIKey",
        Default=Enum.KeyCode.RightShift, Callback=function(k) print(k) end })
    Tab2:AddSeparator()
    Tab2:CreateButton({ Title="Save Config", Callback=function() Win:SaveConfig() end })
    Tab2:CreateButton({ Title="Load Config", Callback=function() Win:LoadConfig() end })

    Win:Notify("KreinGui loaded!", 3)
--]]
...
-- OnClick: InputBegan+InputEnded, responsif di PC & mobile
-- Tambahan: batalkan jika jari bergerak terlalu jauh (itu gesture scroll bukan tap)
local function OnClick(btn, fn)
    local down      = false
    local startPos  = nil
    local DRAG_THRESHOLD = 10  -- pixel, jika lebih dari ini dianggap scroll bukan tap
    ...
    btn.InputEnded:Connect(function(inp)
        if isDown(inp) and down then
            down = false
            -- Cek apakah ini tap atau scroll
            if startPos then
                local delta = (inp.Position - startPos).Magnitude
...
-- ================================================================
-- DRAG (Mouse + Touch, dengan threshold agar tidak konflik scroll)
-- ================================================================
...
function KreinGui:SetTheme(ov)
    ...
    if ov.Accent then
        Theme.AccentHover = Color3.new(
            math.min(ov.Accent.R+0.1,1),
            math.min(ov.Accent.G+0.1,1),
            math.min(ov.Accent.B+0.1,1))
...
function KreinGui:CreateWindow(cfg)
    ...
    -- [FIX] ABar dibuat di Win bukan Header agar tidak muncul saat minimize
    -- ABar posisi tetap di bawah header (y=50), tapi parent Win dan visible diatur
    ...
    ABar.ZIndex = 3  -- di bawah header (ZIndex 4) tapi di atas body
    ...
    if subtitle ~= "" then
        ...
        SL.Font = Enum.Font.Gotham; SL.TextSize = 11; SL.ZIndex = 5
    end
    ...
    CloseBtn.Text = "✕"; CloseBtn.TextSize = 14
    CloseBtn.Font = Enum.Font.GothamBold
    CloseBtn.TextColor3 = Theme.CloseRed
    CloseBtn.ZIndex = 6; CloseBtn.AutoButtonColor = false
    Corner(CloseBtn, 7)
    ...
    MinBtn.ZIndex = 6; MinBtn.AutoButtonColor = false
    Corner(MinBtn, 7)
    ...
    OnClick(MinBtn, function()
        ...
        if minimized then
            -- [FIX] Sembunyikan ABar saat minimize agar tidak ngambang
            Tween(Win, { Size = UDim2.new(0, 560, 0, 52) }, 0.25)
            task.delay(0.1, function() ABar.Visible = false end)
        else
            ABar.Visible = true
    ...
    SepLine.Position = UDim2.new(0, TAB_W, 0, 0)
    SepLine.BackgroundColor3 = Theme.Separator
    ...
    -- ContentPanel: tidak clip agar dropdown/colorpicker popup tidak terpotong
    ...
    function WObj:SaveConfig()
        ...
        for flag, api in pairs(flagMap) do
            ...
            if typeof(v) == "Color3" then
                data[flag] = { __type = "Color3", value = sCol(v) }
        ...
        for flag, val in pairs(data) do
            if flagMap[flag] then
                if type(val) == "table" and val.__type == "Color3" then
                    flagMap[flag]:Set(dCol(val.value))
                elseif type(val) == "table" and val.__type == "EnumItem" then
                    local pts = string.split(val.value, ".")
    ...
    function WObj:Notify(msg, dur)
        ...
        N.BorderSizePixel = 0; N.ZIndex = 200
        ...
        Bar.BorderSizePixel = 0; Bar.ZIndex = 201
        Corner(Bar, 3)
        ...
        nl.ZIndex = 201; nl.TextWrapped = true
    ...
    function WObj:CreateTab(name)
        ...
        Bar.BorderSizePixel = 0; Bar.Visible = false
        Corner(Bar, 2)

        local BL = Lbl(Btn, name, UDim2.new(1, -14, 1, 0), Theme.TabDefaultText)
        BL.Name = "Lbl"
        BL.Position = UDim2.new(0, 10, 0, 0)
        BL.Font = Enum.Font.GothamMedium; BL.TextSize = 12

        OnClick(Btn, function() setActive(idx) end)
        Btn.MouseEnter:Connect(function()
            if active ~= idx then Tween(Btn, { BackgroundColor3 = Theme.TabHover }, 0.15) end
        end)
        Btn.MouseLeave:Connect(function()
            if active ~= idx then Tween(Btn, { BackgroundColor3 = Theme.TabDefault }, 0.15) end
        end)
        tabBtns[idx] = Btn
        ...
        function TObj:CreateButton(cfg2)
            ...
            hov.BorderSizePixel = 0; hov.Text = ""
        ...
        function TObj:CreateSlider(cfg2)
            ...
            TBG.BackgroundColor3 = Theme.ToggleOff
            TBG.BorderSizePixel = 0; Corner(TBG, 5)

            local p0 = (val - mn) / (mx - mn)

            local TFill = Instance.new("Frame", TBG)
            TFill.Size = UDim2.new(p0, 0, 1, 0)
            TFill.BackgroundColor3 = Theme.Accent
            TFill.BorderSizePixel = 0; Corner(TFill, 5)
            ...
            SBtn.Position = UDim2.new(0, 0, 0.5, -20)
            SBtn.BackgroundTransparency = 1
            SBtn.BorderSizePixel = 0; SBtn.Text = ""; SBtn.ZIndex = 4
        ...
        function TObj:CreateTextBox(cfg2)
            ...
            IF.BackgroundColor3 = Theme.WindowBG
            IF.BorderSizePixel = 0; Corner(IF, 6)
        ...
        function TObj:CreateDropdown(cfg2)
            ...
            SF.BackgroundColor3 = Theme.WindowBG
            SF.BorderSizePixel = 0; Corner(SF, 6); Stroke(SF, Theme.ElementStroke, 1)
            ...
            TB2.BorderSizePixel = 0; TB2.Text = ""
            ...
            -- Tutup jika klik di luar (task.defer agar option OnClick keburu fire dulu)
            UserInput.InputBegan:Connect(function(inp)
                if not open or not isDown(inp) then return end
                task.defer(function()
                    if not open then return end
                    local pos = inp.Position
                    local dfP = DF.AbsolutePosition; local dfS = DF.AbsoluteSize
                    local cP  = c.AbsolutePosition;  local cS  = c.AbsoluteSize
                    local inDF = pos.X >= dfP.X and pos.X <= dfP.X + dfS.X and pos.Y >= dfP.Y and pos.Y <= dfP.Y + dfS.Y
                    local inC  = pos.X >= cP.X  and pos.X <= cP.X  + cS.X  and pos.Y >= cP.Y  and pos.Y <= cP.Y  + cS.Y
        ...
        -- ══════════════════════════════════════════════
        -- COLOR PICKER
        -- [FIX] SV box tidak gelap, gradient benar
        -- Tombol OK & Batal
        -- ══════════════════════════════════════════════
        function TObj:CreateColorPicker(cfg2)
            ...
            Pop.Visible = false; Pop.ZIndex = 160
            Corner(Pop, 8); Stroke(Pop, Theme.ElementStroke, 1)

            -- ── SV Box ───────────────────────────────────────
            -- [FIX] Gunakan 3 layer terpisah:
            --   1. Frame warna hue murni (background)
            --   2. Gradient putih horizontal (kiri=putih, kanan=transparan)
            --   3. Gradient hitam vertikal (bawah=hitam, atas=transparan)
            -- Tidak ada ImageLabel rotation yang menyebabkan overlay hitam
            ...
            SVBox.BorderSizePixel = 0; SVBox.ZIndex = 161
            Corner(SVBox, 6)

            -- Layer putih (saturation: kiri putih → kanan transparan)
            local WhiteLayer = Instance.new("Frame", SVBox)
            ...
            WhiteLayer.BorderSizePixel = 0; WhiteLayer.ZIndex = 162
            Corner(WhiteLayer, 6)
            local wg = Instance.new("UIGradient", WhiteLayer)
            wg.Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0, Color3.fromRGB(255,255,255)),
                ColorSequenceKeypoint.new(1, Color3.fromRGB(255,255,255)),
            })
            wg.Transparency = NumberSequence.new({
                NumberSequenceKeypoint.new(0, 0),   -- kiri: putih penuh
                NumberSequenceKeypoint.new(1, 1),   -- kanan: transparan
            })
            wg.Rotation = 0

            -- Layer hitam (value: atas transparan → bawah hitam)
            local BlackLayer = Instance.new("Frame", SVBox)
            BlackLayer.Size = UDim2.new(1, 0, 1, 0)
            BlackLayer.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
            BlackLayer.BackgroundTransparency = 0
            BlackLayer.BorderSizePixel = 0; BlackLayer.ZIndex = 163
            Corner(BlackLayer, 6)
            local bg = Instance.new("UIGradient", BlackLayer)
            bg.Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0, Color3.fromRGB(0,0,0)),
                ColorSequenceKeypoint.new(1, Color3.fromRGB(0,0,0)),
            })
            bg.Transparency = NumberSequence.new({
                NumberSequenceKeypoint.new(0, 1),   -- atas: transparan
                NumberSequenceKeypoint.new(1, 0),   -- bawah: hitam penuh
            })
            bg.Rotation = 90
            ...
            SVCursor.BorderSizePixel = 0; SVCursor.ZIndex = 165
            ...
            HueBar.Position = UDim2.new(0, 8, 0, 124)
            HueBar.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
            HueBar.BorderSizePixel = 0; HueBar.ZIndex = 161
            Corner(HueBar, 4)

            local HueImg = Instance.new("ImageLabel", HueBar)
            HueImg.Size = UDim2.new(1, 0, 1, 0)
            HueImg.BackgroundTransparency = 1; HueImg.BorderSizePixel = 0
            ...
            HueCursor.BorderSizePixel = 0; HueCursor.ZIndex = 163
            Corner(HueCursor, 3); Stroke(HueCursor, Color3.fromRGB(0, 0, 0), 1)

            -- ── Hex display ──────────────────────────────────
            local HexFrame = Instance.new("Frame", Pop)
            HexFrame.Size = UDim2.new(1, -16, 0, 26)
            HexFrame.Position = UDim2.new(0, 8, 0, 148)
            HexFrame.BackgroundColor3 = Theme.WindowBG
            HexFrame.BorderSizePixel = 0; HexFrame.ZIndex = 161
            ...
            HexLbl.Position = UDim2.new(0, 18, 0, 0)
            HexLbl.TextSize = 11; HexLbl.Font = Enum.Font.GothamBold; HexLbl.ZIndex = 162

            -- Preview sementara
            local BotPrev = Instance.new("Frame", Pop)
            BotPrev.Size = UDim2.new(1, -16, 0, 14)
            BotPrev.Position = UDim2.new(0, 8, 0, 180)
            BotPrev.BackgroundColor3 = color
            BotPrev.BorderSizePixel = 0; BotPrev.ZIndex = 161
            Corner(BotPrev, 4)

            -- ── Tombol OK & Batal ─────────────────────────────
            local BtnRow = Instance.new("Frame", Pop)
            BtnRow.Size = UDim2.new(1, -16, 0, 28)
            BtnRow.Position = UDim2.new(0, 8, 0, 200)
            BtnRow.BackgroundTransparency = 1
            BtnRow.BorderSizePixel = 0; BtnRow.ZIndex = 161

            local OKBtn = Instance.new("TextButton", BtnRow)
            OKBtn.Size = UDim2.new(0.5, -3, 1, 0)
            OKBtn.BackgroundColor3 = Theme.Accent
            OKBtn.BorderSizePixel = 0
            OKBtn.Text = "OK"; OKBtn.TextSize = 12
            OKBtn.Font = Enum.Font.GothamBold
            OKBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
            OKBtn.AutoButtonColor = false; OKBtn.ZIndex = 162
            Corner(OKBtn, 6)

            local CancelBtn = Instance.new("TextButton", BtnRow)
            CancelBtn.Size = UDim2.new(0.5, -3, 1, 0)
            CancelBtn.Position = UDim2.new(0.5, 3, 0, 0)
            CancelBtn.BackgroundColor3 = Theme.ElementBGHover
            CancelBtn.BorderSizePixel = 0
            CancelBtn.Text = "Batal"; CancelBtn.TextSize = 12
            CancelBtn.Font = Enum.Font.GothamBold
            CancelBtn.TextColor3 = Theme.TextSecondary
            CancelBtn.AutoButtonColor = false; CancelBtn.ZIndex = 162
            Corner(CancelBtn, 6)
            ...
            local function updatePreview()
                ...
                SVCursor.Position = UDim2.new(tS, -6, 1 - tV, -6)
                HueCursor.Position = UDim2.new(tH, -3, 0, -2)
                HexLbl.Text = toHex(tc)
                BotPrev.BackgroundColor3 = tc
            end
            ...
            local function updatePopPos()
                ...
                local x = math.max(4, math.min(absPos.X - POP_W + 34, vp.X - POP_W - 4))
                local y = absPos.Y + 28
                if y + POP_H > vp.Y - 4 then y = absPos.Y - POP_H - 4 end
            ...
            SVBtn.BorderSizePixel = 0; SVBtn.Text = ""; SVBtn.ZIndex = 166
            ...
            UserInput.InputEnded:Connect(function(inp)
                if isDown(inp) then svDrag = false end
            end)
            ...
            HueBtn.BorderSizePixel = 0; HueBtn.Text = ""; HueBtn.ZIndex = 163
            ...
            UserInput.InputEnded:Connect(function(inp)
                if isDown(inp) then hueDrag = false end
            end)
            ...
            TogBtn.BorderSizePixel = 0; TogBtn.Text = ""
        ...
        function TObj:CreateKeybind(cfg2)
            ...
            Badge.Size = UDim2.new(0, 82, 0, 30)
            Badge.Position = UDim2.new(1, -86, 0.5, -15)
